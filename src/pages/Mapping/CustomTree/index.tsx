...
import { Button, Form, Input, InputAdornment, Space, Switch, Tree } from 'tdesign-react';
...
export default () => {
  ...
  const getLabelContent = (node:any) => {
  ...
  const renderOperations = (node:any) => `value: ${node.value}`;
  ...
  const handleExpand = (vals:any, state:any) => {
    console.info('on expand:', vals, state);
  };

  const handleChange = (vals:any, state:any) => {
    console.info('on change:', vals, state);
  };

  const handleActive = (vals:any, state:any) => {
  ...
  const renderOperations2 = (node:any) => (
    <>
  ...
  const getActiveChecked = () => {
    ...
    console.info(
      'getChecked:',
      nodes.filter((node:any) => node.checked).map((node:any) => node.value),
    );
  };
